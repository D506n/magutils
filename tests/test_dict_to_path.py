...
from magutils.json_path.data_to_path import Path, data_to_paths
...
class TestDictToPathsModeWild:
    ...
    def test_empty_dict(self):
        assert data_to_paths({}) == []

    def test_flat_dict(self):
        assert sorted(data_to_paths({"a": 1, "b": 2})) == ["a", "b"]

    def test_nested_dict(self):
        assert data_to_paths({"a": {"b": 1}}) == ["a.b"]

    def test_deeply_nested_dict(self):
        assert data_to_paths({"a": {"b": {"c": {"d": 1}}}}) == ["a.b.c.d"]

    def test_multiple_branches(self):
        data = {"a": {"x": 1, "y": 2}, "b": {"z": 3}}
        assert sorted(data_to_paths(data)) == ["a.x", "a.y", "b.z"]

    def test_list_wildcard(self):
        data = {"items": [{"id": 1}, {"id": 2}]}
        assert data_to_paths(data) == ["items.*.id"]

    def test_list_of_lists(self):
        data = {"matrix": [[1, 2], [3, 4]]}
        assert data_to_paths(data) == ["matrix.*.*"]

    def test_empty_list(self):
        data = {"items": []}
        assert data_to_paths(data) == ["items.*"]

    def test_mixed_dict_list(self):
        data = {"config": {"users": [{"name": "Alice"}, {"name": "Bob"}], "version": 2}}
        assert sorted(data_to_paths(data)) == ["config.users.*.name", "config.version"]

    def test_list_of_dicts_multiple_keys(self):
        data = {"users": [{"name": "Alice", "age": 30}, {"name": "Bob", "age": 25}]}
        assert sorted(data_to_paths(data)) == ["users.*.age", "users.*.name"]

    def test_nested_list_in_dict(self):
        data = {"groups": [{"tags": ["a", "b"]}]}
        assert data_to_paths(data) == ["groups.*.tags.*"]

    def test_scalar_values(self):
        data = {"int": 1, "str": "hello", "bool": True, "none": None}
        assert sorted(data_to_paths(data)) == ["bool", "int", "none", "str"]
...
class TestDictToPathsModeStrict:
    ...
    def test_list_strict_indices(self):
        data = {"items": [{"id": 1}, {"id": 2}]}
        assert data_to_paths(data, mode="strict") == ["items.0.id", "items.1.id"]

    def test_list_of_lists_strict(self):
        data = {"matrix": [[1, 2], [3, 4]]}
        result = sorted(data_to_paths(data, mode="strict"))
        assert result == ["matrix.0.0", "matrix.0.1", "matrix.1.0", "matrix.1.1"]

    def test_empty_list_strict(self):
        data = {"items": []}
        assert data_to_paths(data, mode="strict") == ["items.*"]
    ...
    def test_flat_dict_strict(self):
        ...
        assert sorted(data_to_paths({"a": 1, "b": 2}, mode="strict")) == ["a", "b"]
...
class TestDictToPathsModeFull:
    ...
    def test_list_full_duplicates(self):
        ...
        data = {"items": [{"id": 1}, {"id": 2}]}
        assert data_to_paths(data, mode="full") == ["items.*.id"]

    def test_list_of_lists_full(self):
        data = {"matrix": [[1, 2], [3, 4]]}
        assert data_to_paths(data, mode="full") == ["matrix.*.*"]

    def test_empty_list_full(self):
        data = {"items": []}
        assert data_to_paths(data, mode="full") == ["items.*"]

    def test_mixed_dict_list_full(self):
        data = {"config": {"users": [{"name": "Alice"}, {"name": "Bob"}], "version": 2}}
        assert sorted(data_to_paths(data, mode="full")) == ["config.users.*.name", "config.version"]

    def test_list_of_dicts_multiple_keys_full(self):
        data = {"users": [{"name": "Alice", "age": 30}, {"name": "Bob", "age": 25}]}
        assert sorted(data_to_paths(data, mode="full")) == ["users.*.age", "users.*.name"]

    def test_nested_list_in_dict_full(self):
        data = {"groups": [{"tags": ["a", "b"]}]}
        assert data_to_paths(data, mode="full") == ["groups.*.tags.*"]

    def test_flat_dict_full(self):
        """Плоский словарь — режим full не влияет."""
        assert sorted(data_to_paths({"a": 1, "b": 2}, mode="full")) == ["a", "b"]
...
class TestDictToPathsEdgeCases:
    ...
    def test_empty_dict_all_modes(self, mode):
        assert data_to_paths({}, mode=mode) == []

    @pytest.mark.parametrize("mode", ["wild", "strict", "full"])
    def test_single_key_all_modes(self, mode):
        assert data_to_paths({"a": 1}, mode=mode) == ["a"]

    @pytest.mark.parametrize("mode", ["wild", "strict", "full"])
    def test_nested_dict_all_modes(self, mode):
        assert data_to_paths({"a": {"b": {"c": 1}}}, mode=mode) == ["a.b.c"]

    @pytest.mark.parametrize("mode", ["wild", "strict", "full"])
    def test_scalar_values_all_modes(self, mode):
        data = {"int": 1, "str": "hello", "bool": True, "none": None}
        assert sorted(data_to_paths(data, mode=mode)) == ["bool", "int", "none", "str"]
