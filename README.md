...
Модуль для работы с JSON-подобными структурами данных (словари и списки) с использованием строковых путей. Поддерживает сложные операции: получение, установка, удаление, форматирование, а также wildcard-обход.
...
- **`'wild'`** (по умолчанию) — все элементы списка схлопываются в один путь с `*`. Подходит для обобщённого обхода.
- **`'strict'`** — каждый элемент списка получает числовой индекс. Позволяет обращаться к конкретным элементам.
...
data = {
    "groups": [
        {"id": 1, "admin": True, "tags": ["a", "b"]},
        {"id": 2, "admin": False, "owner": 10, "tags": ["c"]},
        {"id": 3, "tags": ["d", "e", "f"]},
    ],
}
...
Поведение для пустых списков:
- Во всех режимах пустой список даёт путь `items.*` (индексов нет).

Поведение для плоских словарей:
- Во всех режимах плоский словарь обрабатывается одинаково — `mode` не влияет.
